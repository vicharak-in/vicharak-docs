# Axon Lite Image Flashing Guide

This document provides instructions for flashing OS images to supported storage devices for Axon Lite.
...
- **mmcblk0**: Represents the eMMC on Axon Lite boards, use /dev/mmcblk0 for flashing.
- **mmcblk1**: Represents the SD card on Axon Lite boards, use /dev/mmcblk1 for flashing.
...
#### Windows
**[Using Rkdevtool](https://docs.vicharak.in/vicharak_sbcs/axon-lite/axon-lite-linux/linux-usage-guide/windows-rkdevtool/#flash-raw-image-in-emmc-to-axon-lite-board)**

#### Linux
**[Using Upgrade Tool](https://docs.vicharak.in/vicharak_sbcs/axon-lite/axon-lite-linux/linux-usage-guide/linux-axon-lite-upgrade-tool/#flash-raw-image-in-axon-lite)**
...
When booting from a newly flashed storage device, Axon Lite will perform an initial setup and automatically reboot.

### Unique PARTUUID Enforcement on First Boot

Axon Lite OS images are shipped with the same root filesystem `PARTUUID`. If multiple freshly flashed storage devices are connected, the kernel may mount the rootfs from any one of them, causing conflicts.

On first boot, Axon Lite generates a **unique `PARTUUID`** for the root filesystem to ensure the correct rootfs is mounted after reboot.
