...
- **UART Console**: Users can also access U-Boot logs via the UART interface. Refer to [this guide](https://docs.vicharak.in/vicharak_sbcs/axon-lite/axon-lite-getting-started/#using-serial-console) for instructions on setting up UART.
...
```sh
sudo apt update
sudo apt install u-boot-rk3576-axon-lite
```

### First Boot Behavior

When booting from a newly flashed storage device, Axon Lite will perform an initial setup (first boot configuration) and then automatically reboot.

**Important:** After the reboot, Axon Lite will not automatically boot from the same storage device unless it has been explicitly set as the default. Simply selecting the device once from the boot menu does not persist across reboots.

To ensure Axon Lite boots from the same storage medium after first boot:
