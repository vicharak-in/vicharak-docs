...
.. image:: /_static/images/rk3576-axon-lite/axon-lite-mic.webp
   :width: 70%

The Analog MIC input on the __AXON LITE_LITE_HYPHEN__ board provides a flexible interface for acoustic sensing using a broad class of analog microphone technologies. __AXON LITE_LITE_HYPHEN__ provides support for mics of different types and companies at the same time preserving dynamic range while minimizing noise.
...
* Mic ground → GND (GPIO GND) on `__AXON LITE_LITE_HYPHEN__ Header <https://docs.vicharak.in/vicharak_sbcs/axon-lite/axon-lite-gpio-description/#axon-lite-gpios-header>`_
...
Follow these steps to connect and use an analog microphone with the __AXON LITE_LITE_HYPHEN__ board.

1. Connect the microphone
^^^^^^^^^^^^^^^^^^^^^^^^^

Connect your microphone to the __AXON LITE_LITE_HYPHEN__ board:

* If you are using single interface mic, just connect MIC_OUT to ANALOG_MIC PIN (+) and Microphone ground → __AXON LITE_LITE_HYPHEN__ GND (Connect to GPIO GND)
* Otherwise if you are using differential mic, connect MIC_OUT to ANALOG_MIC PIN (+) and MIC_GND to ANALOG_MIC PIN (-)

Refer to the __AXON LITE_LITE_HYPHEN__ pinout documentation for exact pin locations `__AXON LITE_LITE_HYPHEN__ Pinouts <https://docs.vicharak.in/vicharak_sbcs/axon-lite/peripherals/gpio/#axon-lite-gpios-header>`_

2. Detect the microphone
^^^^^^^^^^^^^^^^^^^^^^^^

Boot the __AXON LITE_LITE_HYPHEN__ board and open a terminal.
...
* Ensure common ground between microphone and __AXON LITE_LITE_HYPHEN__
