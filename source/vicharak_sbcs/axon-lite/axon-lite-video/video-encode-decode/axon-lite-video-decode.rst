...
.. note::                                                                       
   ...
   The RK3576-based Axon Lite board supports hardware encoding via the MPP library. To use hardware encoding on Axon Lite, you must install FFmpeg using commands given below. The generic FFmpeg package does not support Rockchip hardware encoding/decoging.
...
On the Axon Lite Board, FFmpeg is compiled with hardware-accelerated encoding/decoding and RGA filters, leveraging the RK3576 SoC to deliver high-performance processing for high-resolution videos.
...
This command lists all available Rockchip MPP-based hardware decoders. On Axon Lite, supported decoders include H.263, H.264, H.265 (HEVC), MJPEG, VP8 and VP9. 
