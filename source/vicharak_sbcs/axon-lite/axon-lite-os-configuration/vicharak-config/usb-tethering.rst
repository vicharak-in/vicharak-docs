...
The latest version of ``vicharak-config`` introduces **USB Tethering** support for seamless SSH access to your Axon Lite via the **TYPE-C0** USB port. In addition to SSH access, the tethering interface can now **share internet from Ethernet to your host machine** over USB.

Key Features
------------

- Plug-and-play SSH access via USB at a fixed IP (``10.42.0.1``)
- Share internet access over USB when the Axon Lite is connected via Ethernet
...
This feature is fully supported only on the latest Linux kernel. We recommend upgrading to the latest kernel version for proper functionality.
    ...
    .. code::
        ...
        sudo apt install linux-image-6.1.75-axon-lite linux-headers-6.1.75-axon-lite
        sudo reboot
...
Once enabled, connect your host PC to the **TYPE-C0** port of the Axon Lite and SSH directly:
...
If your Axon Lite is connected to the internet via Ethernet, the host PC will also get internet access via the USB connection.
