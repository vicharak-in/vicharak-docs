...
.. warning::
   ...
   - **Kernel Version:** ``5.10.238-axon-lite``
...
The latest OpenWRT image for Axon Lite can be downloaded from the following link:

`OpenWRT Image <https://downloads.vicharak.in/vicharak-axon-lite/openwrt/>`_
...
`Flashing Guide Using Linux_Upgrade_Tool (Linux) </vicharak_sbcs/axon-lite/axon-lite-linux/linux-usage-guide/linux-axon-lite-upgrade-tool/#flash-raw-image-in-axon-lite>`_
...
`Flashing Guide Using Balena Etcher </vicharak_sbcs/axon-lite/axon-lite-linux/linux-usage-guide/rockchip-raw-image/#flashing-with-balena-etcher>`_

Accessing Axon Lite via Wi-Fi
=========================

After booting Axon Lite, the **OpenWRT** Wi-Fi hotspot is automatically enabled.
...
Configuring Axon Lite as a Router
=============================
...
4. Select **Rewrite IP Address** to the interface **end1**, which connects the Axon Lite to the internet via Ethernet.
...
1. **Accessing OpenWRT GUI (LuCI):**
   ...
   If ``http://10.0.0.1/`` does not open or the **OpenWRT** Wi-Fi is not visible, simply reboot the Axon Lite by pressing the **Reset** button.
