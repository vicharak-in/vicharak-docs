...
This comprehensive guide outlines the complete process for building, configuring, and installing the **Xenomai Real-Time Framework** on the **Vicharak Axon Lite** single-board computer using the **Vicharak Linux Kernel 6.1-xenomai** branch. 
...
.. tip::
     ...
     - Vichara Kernel  : **6.1.75-axon-lite**
...
.. tip::
	...
	It is highly recommended to compile **Vicharak kernel** in **Axon Lite** itself and having `TTY To USB Module (UART Module) will be helpful while developing kernel </vicharak_sbcs/axon-lite/axon-lite-getting-started/#using-serial-console>`_.
...
Xenomai requires low-level architectural modifications to support the Rockchip platform architecture integrated into the Vicharak Axon Lite. Apply the patch to the Xenomai source tree.
...
Navigate back to the kernel repository, initialize the default configuration for the **Axon Lite** hardware profile, and enable Xenomai kernel options via the interactive menu configuration utility.
...
1. When prompted by the layout setup script, locate the **Axon Lite** board profile option.
2. Press **3** to select and load the primary Axon Lite hardware configuration definitions.
...
* ``linux-headers-<kernel-version>-axon-lite_<compiled_date>-axon-lite_arm64.deb``
* ``linux-image-<kernel-version>-axon-lite_<compiled_date>-axon-lite_arm64.deb``
* ``linux-libc-dev_<compiled_date>-axon-lite_arm64.deb``

2. Copy to Target Board
=======================

Transfer these files over to your Vicharak Axon Lite hardware platform using Secure Copy (``scp``) or an external flash storage drive.

.. code-block:: bash

    # Example using network transfer
    scp linux-*.deb vicharak@<axon-lite_board_ip_address>:/home/vicharak/

3. Installation Execution
=========================

Log directly into your **Vicharak Axon Lite** command terminal, navigate to the directory holding the transferred packages, and execute the native package manager installation routine:
