...
.. warning::
	To avoid data corruption or an unusable backup, disconnect all peripherals, stop active processes, and if possible perform the backup over UART.
...
5. Read this :ref:`note <backup_options>` and decide which backup option you want to select. Insert a drive in which you want to take your backup into the board. Then, select the required backup option and press ``Enter``.
...
6. Select the drive in which you want to store the backup file. Suppose you want to store backup in ``SanDisk\x203.2Gen1``. Select ``/dev/sda   57.3G   SanDisk\x203.2Gen1   disk`` and press ``Enter``.
...
7. Now, select the partition in which you want to store the backup file, and press ``Enter``.
...
8. You will see Warning Page saying the process will take time. Click on ``yes`` and the backup process will start.

9. Once the Backup is complete you will get the following log in vicharak-config.
   ...
   .. tab-item:: Full Backup Image
      ...
      .. tab-set::
         ...
         .. tab-item:: Linux
            ...
            4) Using ``db`` option upgrade_tool to load the bootloader binary.
      ...
      .. tab-set::
         ...
         .. tab-item:: Linux
            ...
            .. warning::
               ...
               If you are removing the current root filesystem, ensure that you delete files only from the board’s 6th partition and not from any system-related partitions. Do not delete the partition directory itself—remove only the files inside it.

            5) If you want to remove current rootfs from the board then delete all the data from the 6th partition of the mounted disk. Now using ``tar`` command copy the contents of the compressed backup to the device. Then run ``sync`` command flush file system buffer, ensuring all the data is written to eMMC.
